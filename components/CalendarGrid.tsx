    ...
    <div className="w-full bg-slate-900 rounded-3xl shadow-xl overflow-hidden border border-slate-800">
      ...
      <div className="grid grid-cols-7 bg-slate-950/50 border-b border-slate-800">
        {weekDays.map((day, idx) => (
          <div key={day} className={`py-3 text-center text-sm font-bold ${idx === 6 ? 'text-red-400' : 'text-slate-400'}`}>
            {day}
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-7 auto-rows-fr">
        {dates.map((dateInfo, idx) => {
          if (!dateInfo) return <div key={`empty-${idx}`} className="bg-slate-950/30 min-h-[100px] border-b border-r border-slate-800 last:border-r-0"></div>;
          ...
          return (
            <div 
              ...
              className={`
                relative min-h-[100px] p-2 border-b border-r border-slate-800 cursor-pointer transition-all duration-200
                hover:bg-slate-800
                ${isSelected ? 'bg-red-900/20 ring-2 ring-inset ring-red-500 z-10' : ''}
                ${isToday && !isSelected ? 'bg-indigo-900/20' : ''}
              `}
            >
              {/* Solar Day */}
              <div className={`text-xl font-semibold ${isWeekend ? 'text-red-400' : 'text-slate-200'}`}>
                {dateInfo.solar.day}
              </div>

              {/* Lunar Day */}
              <div className="mt-1 flex flex-col items-center">
                 <span className={`text-xs font-medium ${dateInfo.lunar.day === 1 || dateInfo.festival ? 'text-red-400 font-bold' : 'text-slate-500'}`}>
                 ...
                 {dateInfo.isGoodDay && !dateInfo.festival && (
                    <span className="mt-1 w-1.5 h-1.5 rounded-full bg-yellow-500/80" title="Ngày Hoàng Đạo"></span>
                 )}
              </div>
